 ...
 * score down. A large diff reviewed via map-reduce naturally accumulates
 * dozens of low-severity findings across its many chunks — left uncapped, the
 * additive penalty blows past 100 long before a single CRITICAL appears, and
 * the score floors at 0 indistinguishably from a PR full of criticals. Below
 * this cap the score behaves exactly as before (one warning ⇒ 88, etc.);
 * above it, more warnings/suggestions stop moving the number. CRITICAL is
 * deliberately NOT capped — a handful of real criticals should still floor
 * the score, and that signal is correct.
 */
