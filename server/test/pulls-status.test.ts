 ...
 * decides each PR's review STATUS. The DB `status` column holds GitHub's merge
 * state; the review status (needs_review / reviewed / stale) is derived here
 * from head vs lastReviewedSha + age, so it gets unit coverage independent of
 * the route's queries.
 */
